...
class App {
  ...
  onTouchUp(e) {
    ...
    if (this.clickStartX !== undefined && this.clickStartY !== undefined) {
      if (Math.abs(x - this.clickStartX) < 5 && Math.abs(y - this.clickStartY) < 5) {
        if (this.raycast) {
          ...
          if (hits.length > 0) {
            ...
            if (media && media.id && this.navigate) {
              this.navigate('/products', { state: { categoryId: media.id } });
...
export default function CircularGallery({
  ...
  useEffect(() => {
    if (!items || items.length === 0) {
      fetch(`${config.api}/${config.brand}/categories`)
        .then(res => res.json())
        .then(data => {
          const apiItems = data.map(cat => ({
            image: cat.image,
            text: cat.name,
            id: cat._id
          }));
          if (apiItems.length > 0) {
              setFetchedItems(apiItems);
          }
        })
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, { items: fetchedItems, bend, textColor, borderRadius, font, scrollSpeed, scrollEase }, navigate);
    return () => {
      app.destroy();
    };
  }, [fetchedItems, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, navigate]);
