...
export default function DashboardLayout() {
    ...
    const SidebarLink = ({ to, icon, text, end }) => (
        <NavLink 
            ...
            className={({ isActive }) => 
                `group relative flex items-center gap-3 px-5 py-3.5 rounded-2xl transition-all duration-300 font-bold shrink-0 whitespace-nowrap overflow-hidden
                ${isActive 
        ...
        >
            {({ isActive }) => (
                <>
                    {/* Active/Hover Background - Slide in effect */}
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="flex flex-col xl:flex-row gap-6 lg:gap-10 items-start">
                    ...
                    <div className="w-full xl:w-80 shrink-0 flex flex-col gap-6 sticky top-24 z-20">
                        ...
                        <div className="bg-[var(--bg)]/70 backdrop-blur-2xl rounded-3xl p-4 md:p-6 border border-[var(--secondary)]/20 shadow-xl shadow-[var(--secondary)]/5">
                            {/* Mobile Horizontal Scroll, Desktop Vertical */}
                            <nav className="flex flex-row overflow-x-auto xl:flex-col gap-1.5 pb-2 xl:pb-0 no-scrollbar snap-x snap-mandatory">
                                <SidebarLink 
                                    to="/dashboard" 
                                    end={true}
                                    text="Dashboard" 
                                ...
                                <SidebarLink 
                                    to="/dashboard/orders" 
                                    text="My Orders" 
                                    icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 8.25v10.5A2.25 2.25 0 0118 21H6a2.25 2.25 0 01-2.25-2.25V8.25M12 13.5l-3 3m0 0l-3-3m3 3V8.25m3 5.25l3 3m0 0l3-3m-3 3V8.25" /></svg>}
                                />
                                ...
                                <button 
                                    ...
                                    className="group relative flex items-center gap-3 px-5 py-3.5 rounded-2xl text-red-500 font-bold hover:text-white shrink-0 whitespace-nowrap overflow-hidden transition-all duration-300"
                                >
                    ...
                    <div className="flex-1 w-full text-[var(--text)] min-w-0 flex flex-col gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[var(--bg)]/70 backdrop-blur-2xl p-4 md:p-5 rounded-3xl border border-[var(--secondary)]/20 shadow-xl shadow-[var(--secondary)]/5">
                            <div className="flex flex-wrap items-center gap-2 md:gap-3 text-sm font-bold w-full">
                                <button onClick={() => navigate(-1)} className="hover:text-[var(--primary)] transition-all flex items-center gap-1.5 group bg-[var(--secondary)]/10 hover:bg-[var(--secondary)]/20 px-3 md:px-4 py-2 rounded-xl shadow-sm shrink-0">
                                ...
                                <div className="flex items-center gap-2 text-xs md:text-sm flex-1 min-w-0">
                                    <span className="hover:text-[var(--primary)] cursor-pointer tracking-widest opacity-70 transition-colors shrink-0" onClick={() => navigate('/dashboard')}>DASHBOARD</span>
