        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {cart.length === 0 ? (
                <div className="bg-[var(--secondary)]/5 border border-[var(--secondary)]/10 rounded-3xl p-12 text-center flex flex-col items-center justify-center shadow-inner">
                    ...
                    <Link to="/products" className="bg-[var(--primary)] text-[var(--bg)] font-bold px-8 py-3 rounded-xl hover:opacity-90 transition-opacity whitespace-nowrap">
                        Start Shopping
                    </Link>
                </div>
            ) : (
                        ...
                        <div className="flex flex-col gap-6">
                            {cart.map((item) => (
                                <div key={item._id} className="group flex flex-col sm:flex-row items-center gap-6 bg-[var(--bg)] border border-[var(--secondary)]/10 p-4 sm:p-6 rounded-[2rem] shadow-sm hover:shadow-lg transition-all duration-300">
                                    <Link to={`/product/${item._id}`} className="shrink-0 relative h-32 w-32 md:h-36 md:w-36 bg-[var(--secondary)]/5 rounded-2xl overflow-hidden block">
                                    ...
                                    <div className="flex-1 flex flex-col w-full h-full justify-between">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="pr-4">
                                                <h3 className="text-xl font-bold line-clamp-2 group-hover:text-[var(--primary)] transition-colors">{item.name}</h3>
                                                <p className="text-sm opacity-60 font-medium mt-1 uppercase tracking-widest">{item.categoryName || 'Product'}</p>
                                            </div>
                                            <span className="text-xl font-black text-[var(--primary)] shrink-0">₹{item.price}</span>
                                        </div>

                                        <div className="mt-4 pt-4 flex flex-wrap items-center justify-between border-t border-[var(--secondary)]/10 gap-4">
                                            {/* Quantity Stepper */}
                                            <div className="flex items-center gap-4 bg-[var(--secondary)]/5 rounded-full px-4 py-2 border border-[var(--secondary)]/10 shadow-sm">
                                                <button 
                                                    onClick={() => updateCartQuantity(item._id, item.quantity - 1)}
                                                    className="w-6 h-6 flex items-center justify-center font-black text-lg hover:text-[var(--primary)] transition-colors"
                                                >
                                                    -
                                                </button>
                                            ...
                                            <button 
                                                ...
                                                className="text-red-500/80 hover:text-red-500 font-bold text-sm tracking-wider uppercase transition-colors px-3 py-1 flex items-center gap-1 bg-red-500/10 hover:bg-red-500/20 rounded-lg"
                                            >
