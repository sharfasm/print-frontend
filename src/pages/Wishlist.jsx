    ...
    if (wishlist.length === 0) {
        return (
            <section className="py-24 bg-[var(--bg)] text-[var(--text)] min-h-[60vh] flex flex-col items-center justify-center">
                ...
                <Link to="/products" className="bg-[var(--primary)] text-[var(--bg)] font-bold px-8 py-3 rounded-full hover:opacity-90 transition-opacity">
                    Explore Products
                </Link>
            </section>
        );
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    {wishlist.map((item) => (
                        <div key={item._id} className="group relative flex flex-col w-full max-w-[300px] mx-auto sm:max-w-none">
                            <Link to={`/product/${item._id}`} className="block relative aspect-[4/5] overflow-hidden rounded-2xl bg-[var(--secondary)]/5 mb-4 group-hover:shadow-xl transition-all duration-300">
                                <img 
                                    src={item.image} 
                                    alt={item.name}
                            ...
                            <div className="flex flex-col flex-1 pl-1">
                                ...
                                <Link to={`/product/${item._id}`} className="block mb-auto">
